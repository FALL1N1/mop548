...
void FormationMgr::AddCreatureToGroup(uint32 groupId, Creature* member)
{
    ...
    {
        TC_LOG_INFO("entities.unit", "Group found: %u, inserting creature GUID: %u, Group InstanceID %u", groupId, member->GetGUIDLow(), member->GetInstanceId());
        itr->second->AddMember(member);
    }
    //Create new group
    else
    {
        TC_LOG_INFO("entities.unit", "Group not found: %u. Creating new group.", groupId);
...
{
    TC_LOG_INFO("entities.unit", "Deleting member pointer to GUID: %u from group %u", group->GetId(), member->GetDBTableGUIDLow());
    ...
    if (group->isEmpty())
    {
        ...
        TC_LOG_INFO("entities.unit", "Deleting group with InstanceID %u", member->GetInstanceId());
...
{
    TC_LOG_INFO("entities.unit", "CreatureGroup::AddMember: Adding unit GUID: %u.", member->GetGUIDLow());

    //Check if it is a leader
    if (member->GetDBTableGUIDLow() == m_groupID)
    {
        TC_LOG_INFO("entities.unit", "Unit GUID: %u is formation leader. Adding group.", member->GetGUIDLow());
        m_leader = member;
    }
...
void CreatureGroup::MemberAttackStart(Creature* member, Unit* target)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (m_leader) // avoid crash if leader was killed and reset.
            TC_LOG_INFO("entities.unit", "GROUP ATTACK: group instance id %u calls member instid %u", m_leader->GetInstanceId(), member->GetInstanceId());
...
void CreatureGroup::FormationReset(bool dismiss)
{
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (itr->first != m_leader && itr->first->IsAlive())
        {
            ...
            TC_LOG_INFO("entities.unit", "Set %s movement for member GUID: %u", dismiss ? "default" : "idle", itr->first->GetGUIDLow());
        }
    }
    m_Formed = !dismiss;
}
