...
{
    TC_LOG_INFO("outdoorpvp", "Creating capture point %u", entry);
...
bool OPvPCapturePoint::DelCreature(uint32 type)
{
    if (!m_Creatures[type])
    {
        TC_LOG_INFO("outdoorpvp", "opvp creature type %u was already deleted", type);
        return false;
    }
    ...
    TC_LOG_INFO("outdoorpvp", "deleting opvp creature type %u", type);
...
void OutdoorPvP::HandlePlayerLeaveZone(Player* player, uint32 /*zone*/)
{
    ...
    TC_LOG_INFO("outdoorpvp", "Player %s left an outdoorpvp zone", player->GetName().c_str());
}
