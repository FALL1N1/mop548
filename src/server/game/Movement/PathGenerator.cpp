...
PathGenerator::PathGenerator(const Unit* owner) :
    ...
    _navMeshQuery(NULL)
{
    TC_LOG_INFO("maps", "++ PathGenerator::PathGenerator for %u \n", _sourceUnit->GetGUIDLow());
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest)
{
    ...
    TC_LOG_INFO("maps", "++ PathGenerator::CalculatePath() for %u \n", _sourceUnit->GetGUIDLow());
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    {
        TC_LOG_INFO("maps", "++ BuildPolyPath :: (startPoly == 0 || endPoly == 0)\n");
        BuildShortcut();
    ...
    if (farFromPoly)
    {
        TC_LOG_INFO("maps", "++ BuildPolyPath :: farFromPoly distToStartPoly=%.3f distToEndPoly=%.3f\n", distToStartPoly, distToEndPoly);
        ...
        if (_sourceUnit->GetTypeId() == TYPEID_UNIT)
        {
            ...
            {
                TC_LOG_INFO("maps", "++ BuildPolyPath :: underWater case\n");
                if (owner->CanSwim())
                    buildShotrcut = true;
            }
            else
            {
                TC_LOG_INFO("maps", "++ BuildPolyPath :: flying case\n");
                if (owner->CanFly())
                    buildShotrcut = true;
            }
        }
    ...
    {
        TC_LOG_INFO("maps", "++ BuildPolyPath :: (startPoly == endPoly)\n");
        ...
        TC_LOG_INFO("maps", "++ BuildPolyPath :: path type %d\n", _type);
        return;
    }
    ...
    {
        TC_LOG_INFO("maps", "++ BuildPolyPath :: (startPolyFound && endPolyFound)\n");
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        TC_LOG_INFO("maps", "++  m_polyLength=%u prefixPolyLength=%u suffixPolyLength=%u \n", _polyLength, prefixPolyLength, suffixPolyLength);
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    if (pointCount < 2 || dtStatusFailed(dtResult))
    {
        ...
        TC_LOG_INFO("maps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned\n", pointCount);
        BuildShortcut();
        _type = PATHFIND_NOPATH;
        return;
    }
    ...
    {
        TC_LOG_INFO("maps", "++ PathGenerator::BuildPointPath FAILED! path sized %d returned, lower than limit set to %d\n", pointCount, _pointPathLimit);
        BuildShortcut();
    ...
    TC_LOG_INFO("maps", "++ PathGenerator::BuildPointPath path type %d size %d poly-size %d\n", _type, pointCount, _polyLength);
}
...
{
    TC_LOG_INFO("maps", "++ BuildShortcut :: making shortcut\n");
