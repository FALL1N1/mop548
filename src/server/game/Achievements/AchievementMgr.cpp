...
bool AchievementMgr<T>::IsCompletedCriteria(AchievementCriteriaEntry const* achievementCriteria, AchievementEntry const* achievement)
{
    ...
    switch (AchievementCriteriaTypes(achievementCriteria->type))
    {
        ...
        case ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ARCHAEOLOGY_PROJECTS:
            return progress->counter >= achievementCriteria->complete_research_projects.completeCount;
...
void AchievementMgr<Player>::SendAchievementInfo(Player* receiver, uint32 /*achievementId = 0 */) const
{
    ...
    for (CompletedAchievementMap::const_iterator itr = m_completedAchievements.begin(); itr != m_completedAchievements.end(); ++itr)
    {
        ...
        data.WriteBit(achieverGuid[0]);
        data.WriteBit(achieverGuid[2]);
        data.WriteBit(achieverGuid[5]);
        data.WriteBit(achieverGuid[4]);
        data.WriteBit(achieverGuid[3]);
        data.WriteBit(achieverGuid[6]);
        data.WriteBit(achieverGuid[1]);
        data.WriteBit(achieverGuid[7]);

        achievementData.WriteByteSeq(achieverGuid[1]);
        achievementData.WriteByteSeq(achieverGuid[0]);
        achievementData.AppendPackedTime(itr->second.date);
        achievementData << (uint32)0;
        achievementData << (uint32)itr->first;
        achievementData.WriteByteSeq(achieverGuid[7]);
        achievementData.WriteByteSeq(achieverGuid[4]);
        achievementData.WriteByteSeq(achieverGuid[6]);
        achievementData.WriteByteSeq(achieverGuid[2]);
        achievementData.WriteByteSeq(achieverGuid[3]);
        achievementData.WriteByteSeq(achieverGuid[5]);
        achievementData << (uint32)0;
    }
...
bool AchievementMgr<T>::AdditionalRequirementsSatisfied(AchievementCriteriaEntry const* criteria, uint64 miscValue1, uint64 /*miscValue2*/, Unit const* unit, Player* referencePlayer) const
{
    for (uint8 i = 0; i < MAX_ADDITIONAL_CRITERIA_CONDITIONS; ++i)
    {
        ...
        switch (AchievementCriteriaAdditionalCondition(reqType))
        {
            ...
            {
                // miscValue1 is projectId
                ResearchProjectEntry const* projectEntry = sResearchProjectStore.LookupEntry(uint32(miscValue1));
                if (!projectEntry || projectEntry->Rarity != reqValue)
                    return false;
                break;
            }
            case ACHIEVEMENT_CRITERIA_ADDITIONAL_CONDITION_PROJECT_RACE: // 66
            {
                // miscValue1 is projectId
                ResearchProjectEntry const* projectEntry = sResearchProjectStore.LookupEntry(uint32(miscValue1));
                if (!projectEntry || projectEntry->ResearchBranchId != reqValue)
                    return false;
                break;
            }
            default:
