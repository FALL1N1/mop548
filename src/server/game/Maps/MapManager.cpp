...
bool MapManager::CanPlayerEnter(uint32 mapid, Player* player, bool loginCheck)
{
    ...
    if (entry->IsRaid())
    {
        ...
        if ((!group || !group->isRaidGroup()) && !sWorld->getBoolConfig(CONFIG_INSTANCE_IGNORE_RAID))
        {
            ...
            TC_LOG_INFO("maps", "MAP: Player '%s' must be in a raid group to enter instance '%s'", player->GetName().c_str(), mapName);
            return false;
        }
    }
    ...
    if (!player->IsAlive())
    {
        if (Corpse* corpse = player->GetCorpse())
        {
            ...
            if (!corpseMap)
            {
                ...
                TC_LOG_INFO("maps", "MAP: Player '%s' does not have a corpse in instance '%s' and cannot enter.", player->GetName().c_str(), mapName);
                return false;
            }
            TC_LOG_INFO("maps", "MAP: Player '%s' has corpse in instance '%s' and can enter.", player->GetName().c_str(), mapName);
            player->ResurrectPlayer(0.5f, false);
            player->SpawnCorpseBones();
        }
        else
            TC_LOG_INFO("maps", "Map::CanPlayerEnter - player '%s' is dead but does not have a corpse!", player->GetName().c_str());
    }
