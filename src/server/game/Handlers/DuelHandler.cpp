...
void WorldSession::HandleDuelResponseOpcode(WorldPacket& recvPacket)
{
    ...
    if (accepted)
    {
        ...
        //TC_LOG_INFO("network", "WORLD: Received CMSG_DUEL_ACCEPTED");
        TC_LOG_INFO("network", "Player 1 is: %u (%s)", player->GetGUIDLow(), player->GetName().c_str());
        TC_LOG_INFO("network", "Player 2 is: %u (%s)", plTarget->GetGUIDLow(), plTarget->GetName().c_str());
