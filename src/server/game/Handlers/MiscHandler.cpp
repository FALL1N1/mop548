...
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_REPOP_REQUEST Message");
    ...
    {
        TC_LOG_INFO("network", "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated",
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_GOSSIP_SELECT_OPTION");
    ...
    if (IS_CRE_OR_VEH_GUID(guid))
    {
        ...
        if (!unit)
        {
            TC_LOG_INFO("network", "WORLD: HandleGossipSelectOptionOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else if (IS_GAMEOBJECT_GUID(guid))
    {
        ...
        if (!go)
        {
            TC_LOG_INFO("network", "WORLD: HandleGossipSelectOptionOpcode - GameObject (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else
    {
        TC_LOG_INFO("network", "WORLD: HandleGossipSelectOptionOpcode - unsupported GUID type for highguid %u. lowpart %u.", uint32(GUID_HIPART(guid)), uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    {
        TC_LOG_INFO("network", "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
        if (unit)
...
{
    //CMSG_WHO
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_WHO Message");
    ...
    for (uint32 i = 0; i < wordCount; ++i)
    {
        ...
        TC_LOG_INFO("network", "String %u: %s", i, temp.c_str());
    }
    ...
    for (uint32 i = 0; i < zonesCount; ++i)
    {
        ...
        TC_LOG_INFO("network", "Zone %u: %u", i, zoneIds[i]);
    }
    ...
    TC_LOG_INFO("network", "Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask %u, zones %u, strings %u", levelMin, levelMax, playerName.c_str(), guildName.c_str(), raceMask, classMask, zonesCount, wordCount);
    ...
    TC_LOG_INFO("network", "WORLD: Send SMSG_WHO Message");
}

void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %u", GetSecurity());
...
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_PLAYER_LOGOUT Message");
}

void WorldSession::HandleLogoutCancelOpcode(WorldPacket& /*recvData*/)
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_LOGOUT_CANCEL Message");
    ...
    TC_LOG_INFO("network", "WORLD: Sent SMSG_LOGOUT_CANCEL_ACK Message");
}
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Recvd ZONE_UPDATE: %u", newZone);
...
{
    // TC_LOG_INFO("network", "WORLD: Received CMSG_STANDSTATECHANGE"); -- too many spam in log at lags/debug stop
    uint32 animstate;
    recvData >> animstate;
...
void WorldSession::HandleContactListOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_CONTACT_LIST");
    _player->GetSocial()->SendSocialList(_player);
}

void WorldSession::HandleAddFriendOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_ADD_FRIEND");
    ...
    TC_LOG_INFO("network", "WORLD: %s asked to add friend : '%s'",
...
void WorldSession::HandleAddFriendOpcodeCallBack(PreparedQueryResult result, std::string const& friendNote)
{
            ...
            if (friendGuid)
            {
                ...
                else
                {
                    ...
                    if (!GetPlayer()->GetSocial()->AddToSocialList(GUID_LOPART(friendGuid), false))
                    {
                        ...
                        TC_LOG_INFO("network", "WORLD: %s's friend list is full.", GetPlayer()->GetName().c_str());
                    }
                }
    ...
    TC_LOG_INFO("network", "WORLD: Sent (SMSG_FRIEND_STATUS)");
}
...
void WorldSession::HandleDelFriendOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_DEL_FRIEND");
    ...
    TC_LOG_INFO("network", "WORLD: Sent motd (SMSG_FRIEND_STATUS)");
}

void WorldSession::HandleAddIgnoreOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_ADD_IGNORE");
    ...
    TC_LOG_INFO("network", "WORLD: %s asked to Ignore: '%s'",
...
void WorldSession::HandleDelIgnoreOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_DEL_IGNORE");
...
{
    TC_LOG_INFO("network", "CMSG_SET_CONTACT_NOTES");
    uint64 guid;
    std::string note;
    recvData >> guid >> note;
...
void WorldSession::HandleBugOpcode(WorldPacket& recvData)
{
    ...
    if (suggestion == 0)
        TC_LOG_INFO("network", "WORLD: Received CMSG_BUG [Bug Report]");
    else
        TC_LOG_INFO("network", "WORLD: Received CMSG_BUG [Suggestion]");

    TC_LOG_INFO("network", "%s", type.c_str());
    TC_LOG_INFO("network", "%s", content.c_str());
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_RECLAIM_CORPSE");
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_RESURRECT_RESPONSE");
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "CMSG_AREA_TRIGGER. Trigger ID: %u", triggerId);

    Player* player = GetPlayer();
    if (player->IsInFlight())
    {
        TC_LOG_INFO("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, ignore Area Trigger ID:%u",
    ...
    if (!atEntry)
    {
        TC_LOG_INFO("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID:%u",
            player->GetName().c_str(), player->GetGUIDLow(), triggerId);
        return;
    }
    ...
    {
        TC_LOG_INFO("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (trigger map: %u player map: %u), ignore Area Trigger ID: %u",
    ...
    if (atEntry->radius > 0)
    {
        ...
        {
            TC_LOG_INFO("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (radius: %f distance: %f), ignore Area Trigger ID: %u",
    ...
    else
    {
        ...
        {
            TC_LOG_INFO("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (1/2 box X: %f 1/2 box Y: %f 1/2 box Z: %f rotatedPlayerX: %f rotatedPlayerY: %f dZ:%f), ignore Area Trigger ID: %u",
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");
    ...
    if (decompressedSize > 0xFFFF)
    {
        ...
        TC_LOG_INFO("network", "UAD: Account data packet too big, size %u", decompressedSize);
        return;
    }
    ...
    {
        recvData.rfinish();                   // unneeded warning spam in this case
        TC_LOG_INFO("network", "UAD: Failed to decompress account data");
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type;
    type = recvData.ReadBits(3);

    TC_LOG_INFO("network", "RAD: type %u", type);
    ...
    {
        TC_LOG_INFO("network", "RAD: Failed to compress account data");
        return;
    }
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "CMSG_SET_ACTION_BUTTON slotId: %u actionId: %u", slotId, button->id);
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_COMPLETE_CINEMATIC");
}

void WorldSession::HandleNextCinematicCamera(WorldPacket& /*recvData*/)
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_NEXT_CINEMATIC_CAMERA");
}

void WorldSession::HandleMoveTimeSkippedOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_MOVE_TIME_SKIPPED");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_MOVE_FEATHER_FALL_ACK");
...
/*
    ...
    TC_LOG_INFO("network", "WORLD: CMSG_FORCE_MOVE_UNROOT_ACK");
...
/*
    ...
    TC_LOG_INFO("network", "WORLD: CMSG_FORCE_MOVE_ROOT_ACK");
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_INSPECT");
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        TC_LOG_INFO("network", "CMSG_INSPECT_HONOR_STATS: No player found from GUID: " UI64FMTD, (uint64)guid);
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_WORLD_TELEPORT");

    if (GetPlayer()->IsInFlight())
    {
        TC_LOG_INFO("network", "Player '%s' (GUID: %u) in flight, ignore worldport command.",
            GetPlayer()->GetName().c_str(), GetPlayer()->GetGUIDLow());
        return;
    }

    TC_LOG_INFO("network", "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y = %f, z = %f, o = %f",
...
{
    TC_LOG_INFO("network", "Received opcode CMSG_WHOIS");
    std::string charname;
    recvData >> charname;
    ...
    TC_LOG_INFO("network", "Received whois command from player %s for character %s",
        GetPlayer()->GetName().c_str(), charname.c_str());
}

void WorldSession::HandleComplainOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: CMSG_COMPLAIN");
    ...
    TC_LOG_INFO("network", "REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u, unk4 %u, message %s", spam_type, GUID_LOPART(spammer_guid), unk1, unk2, unk3, unk4, description.c_str());
}

void WorldSession::HandleRealmSplitOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "CMSG_REALM_SPLIT");
    ...
    SendPacket(&data);
    //TC_LOG_INFO("response sent %u", unk);
}

void WorldSession::HandleFarSightOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: CMSG_FAR_SIGHT");
    ...
    if (apply)
    {
        TC_LOG_INFO("network", "Added FarSight " UI64FMTD " to player %u", _player->GetUInt64Value(PLAYER_FIELD_FARSIGHT_OBJECT), _player->GetGUIDLow());
    ...
    {
        TC_LOG_INFO("network", "Player %u set vision to self", _player->GetGUIDLow());
...
{
    TC_LOG_INFO("network", "CMSG_SET_TITLE");
...
{
    TC_LOG_INFO("network", "CMSG_TIME_SYNC_RESP");
    ...
    TC_LOG_INFO("network", "Time sync received: counter %u, client ticks %u, time since last sync %u", counter, clientTicks, clientTicks - _player->m_timeSyncClient);

    uint32 ourTicks = clientTicks + (getMSTime() - _player->m_timeSyncServer);

    // diff should be small
    TC_LOG_INFO("network", "Our ticks: %u, diff %u, latency %u", ourTicks, ourTicks - clientTicks, GetLatency());
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_RESET_INSTANCES");
...
{
    TC_LOG_INFO("network", "MSG_SET_DUNGEON_DIFFICULTY");
    ...
    {
        TC_LOG_INFO("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUIDLow(), mode);
        return;
    }
    ...
    {
        TC_LOG_INFO("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: %s, GUID: %u) tried to reset the instance while player is inside!",
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    TC_LOG_INFO("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player %d tried to reset the instance while group member (Name: %s, GUID: %u) is inside!",
...
{
    TC_LOG_INFO("network", "MSG_SET_RAID_DIFFICULTY");
    ...
    if (map && map->IsDungeon())
    {
        TC_LOG_INFO("network", "WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", _player->GetGUIDLow());
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_CANCEL_MOUNT_AURA");
...
{
    // fly mode on/off
    TC_LOG_INFO("network", "WORLD: CMSG_MOVE_SET_CAN_FLY_ACK");
...
{
    /*
        TC_LOG_INFO("network", "WORLD: CMSG_REQUEST_PET_INFO");
        recvData.hexlike();
    */
}
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_SET_TAXI_BENCHMARK_MODE");
    ...
    TC_LOG_INFO("network", "Client used \"/timetest %d\" command", mode);
}
...
void WorldSession::HandleQueryInspectAchievements(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        TC_LOG_INFO("network", "CMSG_QUERY_INSPECT_ACHIEVEMENTS: Inspected Player " UI64FMTD, (uint64)guid);
        return;
    }
...
{
    // empty opcode
    TC_LOG_INFO("network", "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
...
{
    // empty opcode
    TC_LOG_INFO("network", "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
...
void WorldSession::HandleRequestHotfix(WorldPacket& recvPacket)
{
    ...
    for (uint32 i = 0; i < count; ++i)
    {
        ...
        TC_LOG_INFO("network", "SMSG_DB_REPLY: Sent hotfix entry: %u type: %u", entry, type);
    }
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_UPDATE_MISSILE_TRAJECTORY");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_SAVE_CUF_PROFILES");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_REQUEST_RESEARCH_HISTORY");
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_SET_PREFERED_CEMETERY");
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_SET_CURRENCY_FLAGS");
