...
void WorldSession::HandleBattlemasterHelloOpcode(WorldPacket& recvData)
{
    uint64 guid;
    recvData >> guid;
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEMASTER_HELLO Message from (GUID: %u TypeId:%u)", GUID_LOPART(guid), GuidHigh2TypeId(GUID_HIPART(guid)));
...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket& recvData)
{
    ...
    //TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEMASTER_JOIN Message from (GUID:"UI64FMTD" TypeId:%u)", guid, bgTypeId_);
    ...
    if (!asGroup)
    {
        ...
        TC_LOG_INFO("bg.battleground", "Battleground: player joined queue for bg queue type %u bg type %u: GUID %u, NAME %s",
    ...
    else
    {
        ...
        if (!err)
        {
            TC_LOG_INFO("bg.battleground", "Battleground: the following players are joining as group:");
        ...
        TC_LOG_INFO("bg.battleground", "Battleground: group end");
    }
...
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEGROUND_PLAYER_POSITIONS Message");
...
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_PVP_LOG_DATA Message");
    ...
    TC_LOG_INFO("network", "WORLD: Sent SMSG_PVP_LOG_DATA Message");
}

void WorldSession::HandleBattlefieldListOpcode(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEFIELD_LIST Message");
    ...
    if (!bl)
    {
        TC_LOG_INFO("bg.battleground", "BattlegroundHandler: invalid bgtype (%u) with player (Name: %s, GUID: %u) received.", bgTypeId, _player->GetName().c_str(), _player->GetGUIDLow());
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEFIELD_PORT Message");
    ...
    {
        TC_LOG_INFO("bg.battleground", "CMSG_BATTLEFIELD_PORT %s Slot: %u, Unk: %u, Time: %u, Action: %u. Player not in queue!",
    ...
    {
        TC_LOG_INFO("bg.battleground", "CMSG_BATTLEFIELD_PORT %s Slot: %u, Unk: %u, Time: %u, Action: %u. Invalid queueSlot!",
            GetPlayerInfo().c_str(), queueSlot, id, time, action);
        return;
    }
    ...
    {
        TC_LOG_INFO("bg.battleground", "CMSG_BATTLEFIELD_PORT %s Slot: %u, Unk: %u, Time: %u, Action: %u. Player not in queue (No player Group Info)!",
            GetPlayerInfo().c_str(), queueSlot, id, time, action);
        return;
    }
    ...
    {
        TC_LOG_INFO("bg.battleground", "CMSG_BATTLEFIELD_PORT %s Slot: %u, Unk: %u, Time: %u, Action: %u. Player is not invited to any bg!",
            GetPlayerInfo().c_str(), queueSlot, id, time, action);
        return;
    }
    ...
    if (!bg)
    {
        if (action)
        {
            TC_LOG_INFO("bg.battleground", "CMSG_BATTLEFIELD_PORT %s Slot: %u, Unk: %u, Time: %u, Action: %u. Cant find BG with id %u!",
    ...
    TC_LOG_INFO("bg.battleground", "CMSG_BATTLEFIELD_PORT %s Slot: %u, Unk: %u, Time: %u, Action: %u.",
        GetPlayerInfo().c_str(), queueSlot, id, time, action);
    ...
    if (action == 1 && ginfo.ratedType == 0)
    {
        ...
        if (!_player->CanJoinToBattleground(bg))
        {
            ...
            action = 0;
            TC_LOG_INFO("bg.battleground", "Player %s (%u) has a deserter debuff, do not port him to battleground!", _player->GetName().c_str(), _player->GetGUIDLow());
        }
        //if player don't match battleground max level, then do not allow him to enter! (this might happen when player leveled up during his waiting in queue
        if (_player->getLevel() > bg->GetMaxLevel())
        {
            TC_LOG_INFO("network", "Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do not port him to battleground!",
    ...
    if (action)
    {
        ...
        TC_LOG_INFO("bg.battleground", "Battleground: player %s (%u) joined battle for bg %u, bgtype %u, queue type %u.", _player->GetName().c_str(), _player->GetGUIDLow(), bg->GetInstanceID(), bg->GetTypeID(), bgQueueTypeId);
    }
    else // leave queue
    {
        ...
        if (ginfo.isRated && ginfo.isInvitedToBGInstanceGUID)
        {
            ...
            TC_LOG_INFO("bg.battleground", "UPDATING memberLost's personal arena rating for %u by opponents rating: %u, because he has left queue!", GUID_LOPART(_player->GetGUID()), ginfo.opponentsTeamRating);
        }
        ...
        TC_LOG_INFO("bg.battleground", "Battleground: player %s (%u) left queue for bgtype %u, queue type %u.", _player->GetName().c_str(), _player->GetGUIDLow(), bg->GetTypeID(), bgQueueTypeId);
    }
}

void WorldSession::HandleBattlefieldLeaveOpcode(WorldPacket& /*recvData*/)
{
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEFIELD_LEAVE Message");
...
{
    // empty opcode
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_BATTLEFIELD_STATUS Message");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_BATTLEMASTER_JOIN_ARENA");
    ...
    if (!err)
    {
        TC_LOG_INFO("bg.battleground", "Battleground: Group (GUID: %u), Leader (%s, GUID: %u) just queued arena (Average MMR: %u, Average Rating: %u, Type %u)",
    ...
    for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
    {
        ...
        TC_LOG_INFO("bg.battleground", "Battleground: player joined queue for arena as group bg queue type %u bg type %u: GUID %u, NAME %s", bgQueueTypeId, bgTypeId, member->GetGUIDLow(), member->GetName().c_str());
    }
...
void WorldSession::HandleReportPvPAFK(WorldPacket& recvData)
{
    ...
    if (!reportedPlayer)
    {
        TC_LOG_INFO("bg.battleground", "WorldSession::HandleReportPvPAFK: player not found");
        return;
    }

    TC_LOG_INFO("bg.battleground", "WorldSession::HandleReportPvPAFK: %s reported %s", _player->GetName().c_str(), reportedPlayer->GetName().c_str());
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_REQUEST_RATED_INFO");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_REQUEST_PVP_OPTIONS_ENABLED");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_REQUEST_PVP_REWARDS");
