...
void WorldSession::HandleQuestgiverStatusQueryOpcode(WorldPacket& recvData)
{
    ...
    if (!questgiver)
    {
        TC_LOG_ERROR("network", "Error in CMSG_QUESTGIVER_STATUS_QUERY, called for non-existing questgiver (Typeid: %u GUID: %u)", GuidHigh2TypeId(GUID_HIPART(guid)), GUID_LOPART(guid));
        return;
    }
    ...
    case TYPEID_UNIT:
        {
            TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for npc, guid = %u", uint32(GUID_LOPART(guid)));
    ...
    case TYPEID_GAMEOBJECT:
        {
            TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_STATUS_QUERY for GameObject guid = %u", uint32(GUID_LOPART(guid)));
...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_HELLO npc = %u", GUID_LOPART(guid));

    Creature* creature = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_NONE);
    if (!creature)
    {
        TC_LOG_INFO("network", "WORLD: HandleQuestgiverHelloOpcode - Unit (GUID: %u) not found or you can't interact with him.",
            GUID_LOPART(guid));
        return;
    }
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_ACCEPT_QUEST npc = %u, quest = %u", uint32(GUID_LOPART(guid)), questId);
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    recvData >> questId;
    recvData.ReadGuidMask(guid, 2, 6, 5, 0, 4, 3, 1, 7);
    recvData.ReadBit();

    recvData.ReadGuidBytes(guid, 2, 0, 4, 7, 5, 1, 3, 6);

    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_QUERY_QUEST npc = %u, quest = %u", uint32(GUID_LOPART(guid)), questId);
...
void WorldSession::HandleQuestQueryOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUEST_QUERY quest = %u", questId);
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_CHOOSE_REWARD npc = %u, quest = %u, reward = %u", uint32(GUID_LOPART(guid)), questId, reward);
    ...
    if (!quest->IsRewChoiceItemValid(reward, _player->getClass()))
    {
...
void WorldSession::HandleQuestgiverRequestRewardOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_REQUEST_REWARD npc = %u, quest = %u", uint32(GUID_LOPART(guid)), questId);
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_CANCEL");
...
void WorldSession::HandleQuestLogSwapQuest(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTLOG_SWAP_QUEST slot 1 = %u, slot 2 = %u", slot1, slot2);
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTLOG_REMOVE_QUEST slot = %u", slot);
...
void WorldSession::HandleQuestConfirmAccept(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUEST_CONFIRM_ACCEPT questId = %u", questId);
...
void WorldSession::HandleQuestgiverCompleteQuest(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_COMPLETE_QUEST npc = %u, questId = %u", uint32(GUID_LOPART(playerGuid)), questId);
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_QUEST_AUTOLAUNCH");
}
...
void WorldSession::HandlePushQuestToParty(WorldPacket& recvPacket)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_PUSH_QUEST_TO_PARTY questId = %u", questId);
...
void WorldSession::HandleQuestPushResult(WorldPacket& recvPacket)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received MSG_QUEST_PUSH_RESULT");
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_QUESTGIVER_STATUS_MULTIPLE_QUERY");
    ...
    for (Player::ClientGUIDs::const_iterator itr = _player->m_clientGUIDs.begin(); itr != _player->m_clientGUIDs.end(); ++itr)
    {
        ...
        if (IS_CRE_OR_VEH_OR_PET_GUID(*itr))
        {
            ...
            data.WriteGuidMask(guid, 4, 0, 3, 6, 5, 7, 1, 2);

            byteData.WriteGuidBytes(guid, 6, 2, 7, 5, 4);
            byteData << uint32(questStatus);
            byteData.WriteGuidBytes(guid, 1, 3, 0);
