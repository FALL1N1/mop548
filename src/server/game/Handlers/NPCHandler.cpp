...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleTabardVendorActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleBankerActivateOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_BANKER_ACTIVATE");
    ...
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleBankerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: SendTrainerList");

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: SendTrainerList - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    if (!ci)
    {
        TC_LOG_INFO("network", "WORLD: SendTrainerList - (GUID: %u) NO CREATUREINFO!", GUID_LOPART(guid));
        return;
    }
    ...
    if (!trainer_spells)
    {
        TC_LOG_INFO("network", "WORLD: SendTrainerList - Training spells not found for creature (GUID: %u Entry: %u)",
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Received CMSG_TRAINER_BUY_SPELL NpcGUID=%u, learn spell id is: %u", uint32(GUID_LOPART(guid)), spellId);

    Creature* unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TRAINER);
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleTrainerBuySpellOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_GOSSIP_HELLO");
    ...
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleGossipHelloOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
/*void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData)
{
    ...
    {
        TC_LOG_INFO("network", "reading string");
        recvData >> code;
        TC_LOG_INFO("network", "string read: %s", code.c_str());
    }
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_SPIRIT_HEALER_ACTIVATE");
    ...
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleSpiritHealerActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleBinderActivateOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleBinderActivateOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(npcGuid)));
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: Recv MSG_LIST_STABLED_PETS");
    ObjectGuid npcGUID;
...
void WorldSession::SendStablePetCallback(PreparedQueryResult result, uint64 guid)
{
    ...
    TC_LOG_INFO("network", "WORLD: Recv MSG_LIST_STABLED_PETS Send.");
...
{
    TC_LOG_INFO("network", "WORLD: Recv CMSG_STABLE_PET");
    uint32 PetGUID;
    uint8 petnumber;
    ObjectGuid npcGUID;
...
{
    TC_LOG_INFO("network", "WORLD: Recv CMSG_STABLE_CHANGE_SLOT");
    uint32 PetGUID;
    uint8 petnumber;
    ObjectGuid npcGUID;
    recvData >> PetGUID >> petnumber;
...
{
    TC_LOG_INFO("network", "WORLD: Recv CMSG_UNSTABLE_PET.");
    uint64 npcGUID;
    uint32 petnumber;
...
{
    TC_LOG_INFO("network", "WORLD: Recv CMSG_BUY_STABLE_SLOT.");
    uint64 npcGUID;
...
{
    TC_LOG_INFO("network", "HandleStableRevivePet: Not implemented");
}

void WorldSession::HandleStableSwapPet(WorldPacket& recvData)
{
    TC_LOG_INFO("network", "WORLD: Recv CMSG_STABLE_SWAP_PET.");
    uint64 npcGUID;
    uint32 petId;
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_REPAIR_ITEM");
    ...
    if (!unit)
    {
        TC_LOG_INFO("network", "WORLD: HandleRepairItemOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(npcGuid)));
        return;
    }
    ...
    if (itemGuid)
    {
        TC_LOG_INFO("network", "ITEM: Repair item, itemGUID = %u, npcGUID = %u", GUID_LOPART(itemGuid), GUID_LOPART(npcGuid));
    ...
    {
        TC_LOG_INFO("network", "ITEM: Repair all items, npcGUID = %u", GUID_LOPART(npcGuid));
