...
{
    //TC_LOG_INFO("network", "WORLD: CMSG_SPLIT_ITEM");
    ...
    //TC_LOG_INFO("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u, dstslot = %u, count = %u", srcbag, srcslot, dstbag, dstslot, count);
...
{
    //TC_LOG_INFO("network", "WORLD: CMSG_SWAP_INV_ITEM");
    uint8 srcSlot, dstSlot;

    recvData >> srcSlot >> dstSlot;
    //TC_LOG_INFO("STORAGE: receive srcslot = %u, dstslot = %u", srcslot, dstslot);
...
{
    //TC_LOG_INFO("network", "WORLD: CMSG_SWAP_ITEM");
    bool hasSlot[2];
    bool hasBag[2];
    ...
    //TC_LOG_INFO("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u, dstslot = %u", srcbag, srcslot, dstbag, dstslot);
...
{
    //TC_LOG_INFO("network", "WORLD: CMSG_AUTOEQUIP_ITEM");
    uint8 srcbag, srcslot;

    recvData >> srcslot >> srcbag;
    recvData.rfinish();
    //TC_LOG_INFO("STORAGE: receive srcbag = %u, srcslot = %u", srcbag, srcslot);
...
{
    //TC_LOG_INFO("network", "WORLD: CMSG_DESTROY_ITEM");
    int32 count;
    int8 bag, slot;

    recvData >> count;
    recvData >> slot >> bag;
    //TC_LOG_INFO("STORAGE: receive bag = %u, slot = %u, count = %u", bag, slot, count);
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_SELL_ITEM");
    ...
    if (!creature)
    {
        TC_LOG_INFO("network", "WORLD: HandleSellItemOpcode - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorGuid)));
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_BUYBACK_ITEM");
    ...
    if (!creature)
    {
        TC_LOG_INFO("network", "WORLD: HandleBuybackItem - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorGuid)));
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_BUY_ITEM_IN_SLOT");
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_BUY_ITEM");
    ...
    if (itemType == ITEM_VENDOR_TYPE_ITEM)
    {
        ...
        TC_LOG_INFO("network", "WORLD: received wrong itemType (%u) in HandleBuyItemOpcode", itemType);
}
...
void WorldSession::HandleListInventoryOpcode(WorldPacket& recvData)
{
    ...
    TC_LOG_INFO("network", "WORLD: Recvd CMSG_LIST_INVENTORY");
...
{
    TC_LOG_INFO("network", "WORLD: Sent SMSG_LIST_INVENTORY");

    Creature* vendor = GetPlayer()->GetNPCIfCanInteractWith(vendorGuid, UNIT_NPC_FLAG_VENDOR);
    if (!vendor)
    {
        TC_LOG_INFO("network", "WORLD: SendListInventory - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorGuid)));
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, NULL, 0);
        return;
    }
...
{
    //TC_LOG_INFO("network", "WORLD: CMSG_AUTOSTORE_BAG_ITEM");
    uint8 srcBag, srcSlot, dstBag;

    recvData >> srcSlot >> srcBag >> dstBag;
    recvData.rfinish();

    //TC_LOG_INFO("STORAGE: receive srcbag = %u, srcslot = %u, dstbag = %u", srcbag, srcslot, dstbag);
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_BUY_BANK_SLOT");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_AUTOBANK_ITEM");
    uint8 srcBag, srcSlot;

    recvPacket >> srcSlot >> srcBag;
    recvPacket.rfinish();

    TC_LOG_INFO("network", "STORAGE: receive srcbag = %u, srcslot = %u", srcBag, srcSlot);
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_AUTOSTORE_BANK_ITEM");
    uint8 srcBag, srcSlot;
...
{
    TC_LOG_INFO("network", "Received opcode CMSG_WRAP_ITEM");

    uint8 gift_bag = 0, gift_slot = 0, item_bag = 0, item_slot = 0;

    uint8 item_count = uint8(recvData.ReadBits(2));

    TC_LOG_INFO("network", "WRAP: receive item_count = %u", item_count);
    ...
    TC_LOG_INFO("network", "WRAP: receive gift_bag = %u, gift_slot = %u, item_bag = %u, item_slot = %u", gift_bag, gift_slot, item_bag, item_slot);
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_SOCKET_GEMS");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_CANCEL_TEMP_ENCHANTMENT");
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_ITEM_REFUND_INFO");
    ...
    if (!item)
    {
        TC_LOG_INFO("network", "Item refund: item not found!");
        return;
    }
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_ITEM_REFUND");
    uint64 guid;
...
void WorldSession::HandleItemTextQuery(WorldPacket& recvData )
{
    ...
    TC_LOG_INFO("network", "CMSG_ITEM_TEXT_QUERY item guid: %u", GUID_LOPART(itemGuid));
...
{
    TC_LOG_INFO("network", "WORLD: Received CMSG_TRANSMOGRIFY_ITEMS");
    ...
    {
        TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) sent a wrong count (%u) when transmogrifying items.", player->GetGUIDLow(), player->GetName().c_str(), count);
        recvData.rfinish();
        return;
    }
    ...
    {
        TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Unit (GUID: %u) not found or player can't interact with it.", GUID_LOPART(npcGuid));
        return;
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        {
            TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an item (lowguid: %u) with a wrong slot (%u) when transmogrifying items.", player->GetGUIDLow(), player->GetName().c_str(), GUID_LOPART(itemGuids[i]), slots[i]);
            return;
        }
        ...
        if (newEntries[i])
        {
            ...
            if (!proto)
            {
                TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify to an invalid item (entry: %u).", player->GetGUIDLow(), player->GetName().c_str(), newEntries[i]);
                return;
            }
        }
        ...
        if (itemGuids[i])
        {
            ...
            {
                TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (lowguid: %u).", player->GetGUIDLow(), player->GetName().c_str(), GUID_LOPART(itemGuids[i]));
                return;
            }
        }
        ...
        {
            TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an invalid item in a valid slot (slot: %u).", player->GetGUIDLow(), player->GetName().c_str(), slots[i]);
            return;
        }
        ...
        //{
        //    TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) can't equip the item to be transmogrified (slot: %u, entry: %u).", player->GetGUIDLow(), player->GetName().c_str(), slots[i], itemTransmogrified->GetEntry());
        //    return;
        //}
        //
        ...
        //{
        //    TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) can't equip the transmogrifier item (slot: %u, entry: %u).", player->GetGUIDLow(), player->GetName().c_str(), slots[i], itemTransmogrifier->GetEntry());
        //    return;
        //}
        ...
        else
        {
            ...
            {
                TC_LOG_INFO("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) failed CanTransmogrifyItemWithItem (%u with %u).", player->GetGUIDLow(), player->GetName().c_str(), itemTransmogrified->GetEntry(), itemTransmogrifier->GetEntry());
                return;
            }
...
{
    TC_LOG_INFO("network", "WORLD: CMSG_UPGRADE_ITEM");
    ...
    TC_LOG_INFO("network", "WORLD: HandleUpgradeItem ItemPos: %u, Bag: %u, ItemUpgradeId: %u", ItemPos, Bag, ItemUpgradeId);
...
{
    TC_LOG_INFO("network", "WORLD: SMSG_ITEM_UPGRADE_RESULT");
