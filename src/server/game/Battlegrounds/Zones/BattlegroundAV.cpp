...
{
    TC_LOG_INFO("bg.battleground", "bg_av HandleKillUnit %i", unit->GetEntry());
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    TC_LOG_INFO("bg.battleground", "BG_AV Quest %i completed", questid);
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            {
                TC_LOG_INFO("bg.battleground", "BG_AV Quest %i completed starting with unit upgrading..", questid);
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            if (m_Team_QuestStatus[team][1] == 30)
                TC_LOG_INFO("bg.battleground", "BG_AV Quest %i completed (need to implement some events here", questid);
            break;
        ...
        case AV_QUEST_H_NEAR_MINE:
            ...
            if (m_Team_QuestStatus[team][5] == 28)
            {
                ...
                if (m_Team_QuestStatus[team][6] == 7)
                    TC_LOG_INFO("bg.battleground", "BG_AV Quest %i completed (need to implement some events here - ground assault ready", questid);
            }
            break;
        ...
        case AV_QUEST_H_RIDER_HIDE:
            ...
            if (m_Team_QuestStatus[team][7] == 25)
            {
                ...
                if (m_Team_QuestStatus[team][8] == 25)
                    TC_LOG_INFO("bg.battleground", "BG_AV Quest %i completed (need to implement some events here - rider assault ready", questid);
            }
            break;
        ...
        case AV_QUEST_H_RIDER_TAME:
            ...
            break;
        default:
            TC_LOG_INFO("bg.battleground", "BG_AV Quest %i completed but is not interesting at all", questid);
...
{
    TC_LOG_INFO("bg.battleground", "BG_AV: start spawning mine stuff");
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    TC_LOG_INFO("bg.battleground", "bg_av: player destroyed point node %i object %i", node, object);
...
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial)
{
    ...
    if (!initial)
    {
        TC_LOG_INFO("bg.battleground", "bg_av depopulating mine %i (0=north, 1=south)", mine);
    ...
    TC_LOG_INFO("bg.battleground", "bg_av populating mine %i (0=north, 1=south)", mine);
    uint16 miner;
    ...
    else
    {
       ...
       //vermin
        TC_LOG_INFO("bg.battleground", "spawning vermin");
        if (team == ALLIANCE)
...
{
    TC_LOG_INFO("bg.battleground", "bg_AV getnodethroughobject %i", object);
...
{ //this function is the counterpart to GetNodeThroughObject()
    TC_LOG_INFO("bg.battleground", "bg_AV GetObjectThroughNode %i", node);
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    ...
    TC_LOG_INFO("bg.battleground", "BG_AV using gameobject %i with type %i", target_obj->GetEntry(), object);
    if (object < 0)
        return;
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    TC_LOG_INFO("bg.battleground", "player defends point object: %i node: %i", object, node);
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    uint32 team  = player->GetTeam();
    TC_LOG_INFO("bg.battleground", "bg_av: player assaults point object %i node %i", object, node);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    uint16 i;
    TC_LOG_INFO("bg.battleground", "Alterac Valley: entering state STATUS_WAIT_JOIN ...");
    // Initial Nodes
    ...
    //creatures
    TC_LOG_INFO("bg.battleground", "BG_AV start poputlating nodes");
    for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        if (m_Nodes[i].Owner)
            PopulateNode(i);
    ...
    TC_LOG_INFO("bg.battleground", "BG_AV: start spawning static creatures");
    for (i = 0; i < AV_STATICCPLACE_MAX; i++)
        AddAVCreature(0, i + AV_CPLACE_MAX);
    //mainspiritguides:
    TC_LOG_INFO("bg.battleground", "BG_AV: start spawning spiritguides creatures");
    ...
    TC_LOG_INFO("bg.battleground", "BG_AV: start spawning marshal creatures");
