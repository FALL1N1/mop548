...
void BattlegroundBFG::_CreateBanner(uint8 node, uint8 type, uint8 teamIndex, bool delay)
{
    ...
    object = node * 8 + ((type == BG_BFG_NODE_TYPE_OCCUPIED) ? (3 + teamIndex) : 5);
    SpawnBGObject(object, RESPAWN_IMMEDIATELY);
}
...
bool BattlegroundBFG::SetupBattleground()
{
    for (int i = 0; i < BG_BFG_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_BFG_OBJECT_BANNER_NEUTRAL + 8 * i, BG_BFG_OBJECTID_NODE_BANNER_0 + i * 3, BG_BFG_NodePositions[i][0], BG_BFG_NodePositions[i][1], BG_BFG_NodePositions[i][2], BG_BFG_NodePositions[i][3], 0, 0, sin(BG_BFG_NodePositions[i][3] / 2), cos(BG_BFG_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
        ...
        {
            TC_LOG_ERROR("sql.sql", "BattleForGilneas: Can't Create Some Object");
            return false;
        }
    }
    ...
    {
        TC_LOG_ERROR("sql.sql", "BattleForGilneas: Can't Create Doors");
        return false;
    }
    ...
    for (int i = 0; i < BG_BFG_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_BFG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i, Buff_Entries[0], BG_BFG_BuffPositions[i][0], BG_BFG_BuffPositions[i][1], BG_BFG_BuffPositions[i][2], BG_BFG_BuffPositions[i][3], 0, 0, sin(BG_BFG_BuffPositions[i][3] / 2), cos(BG_BFG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY)
            ...
            TC_LOG_ERROR("sql.sql", "BattleForGilneas: Can't Create Buffs");
    }
