...
void BattlegroundTP::RespawnFlag(uint32 Team, bool captured)
{
    if (Team == ALLIANCE)
    {
        TC_LOG_INFO("bg.battleground", "Respawn Alliance flag");
        _flagState[TEAM_ALLIANCE] = BG_TP_FLAG_STATE_ON_BASE;
    }
    else
    {
        TC_LOG_INFO("bg.battleground", "Respawn Horde flag");
...
bool BattlegroundTP::SetupBattleground()
{
    ...
    TC_LOG_INFO("bg.battleground", "BatteGroundTP: BG objects and spirit guides spawned");
