...
void LFGGroupScript::OnAddMember(Group* group, uint64 guid)
{
    ...
    if (leader == guid)
    {
        TC_LOG_INFO("lfg", "LFGScripts::OnAddMember [" UI64FMTD "]: added [" UI64FMTD "] leader " UI64FMTD "]", gguid, guid, leader);
    ...
    else
    {
        ...
        TC_LOG_INFO("lfg", "LFGScripts::OnAddMember [" UI64FMTD "]: added [" UI64FMTD "] leader " UI64FMTD "] gstate: %u, state: %u", gguid, guid, leader, gstate, state);
...
void LFGGroupScript::OnRemoveMember(Group* group, uint64 guid, RemoveMethod method, uint64 kicker, char const* reason)
{
    ...
    uint64 gguid = group->GetGUID();
    TC_LOG_INFO("lfg", "LFGScripts::OnRemoveMember [" UI64FMTD "]: remove [" UI64FMTD "] Method: %d Kicker: [" UI64FMTD "] Reason: %s", gguid, guid, method, kicker, (reason ? reason : ""));
...
void LFGGroupScript::OnDisband(Group* group)
{
    ...
    uint64 gguid = group->GetGUID();
    TC_LOG_INFO("lfg", "LFGScripts::OnDisband [" UI64FMTD "]", gguid);
...
void LFGGroupScript::OnChangeLeader(Group* group, uint64 newLeaderGuid, uint64 oldLeaderGuid)
{
    ...
    TC_LOG_INFO("lfg", "LFGScripts::OnChangeLeader [" UI64FMTD "]: old [" UI64FMTD "] new [" UI64FMTD "]", gguid, newLeaderGuid, oldLeaderGuid);
...
void LFGGroupScript::OnInviteMember(Group* group, uint64 guid)
{
    ...
    uint64 gguid = group->GetGUID();
    uint64 leader = group->GetLeaderGUID();
    TC_LOG_INFO("lfg", "LFGScripts::OnInviteMember [" UI64FMTD "]: invite [" UI64FMTD "] leader [" UI64FMTD "]", gguid, guid, leader);
