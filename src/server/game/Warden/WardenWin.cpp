...
void WardenWin::Init(WorldSession* session, BigNumber* k)
{
    ...
    TC_LOG_INFO("warden", "Server side warden for client %u initializing...", session->GetAccountId());
    TC_LOG_INFO("warden", "C->S Key: %s", ByteArrayToHexStr(_inputKey, 16).c_str());
    TC_LOG_INFO("warden", "S->C Key: %s", ByteArrayToHexStr(_outputKey, 16).c_str());
    TC_LOG_INFO("warden", "  Seed: %s", ByteArrayToHexStr(_seed, 16).c_str());
    TC_LOG_INFO("warden", "Loading Module...");

    _module = GetModuleForClient();

    TC_LOG_INFO("warden", "Module Key: %s", ByteArrayToHexStr(_module->Key, 16).c_str());
    TC_LOG_INFO("warden", "Module ID: %s", ByteArrayToHexStr(_module->Id, 16).c_str());
    RequestModule();
}
...
{
    TC_LOG_INFO("warden", "Initialize module");
...
{
    TC_LOG_INFO("warden", "Request hash");
...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    TC_LOG_INFO("warden", "Request hash reply: succeed");
...
{
    TC_LOG_INFO("warden", "Request data");
    ...
    TC_LOG_INFO("warden", "%s", stream.str().c_str());
}

void WardenWin::HandleData(ByteBuffer &buff)
{
    TC_LOG_INFO("warden", "Handle data");
    ...
    // TIMING_CHECK
    {
        ...
        TC_LOG_INFO("warden", "ServerTicks %u", ticksNow);         // Now
        TC_LOG_INFO("warden", "RequestTicks %u", _serverTicks);    // At request
        TC_LOG_INFO("warden", "Ticks %u", newClientTicks);         // At response
        TC_LOG_INFO("warden", "Ticks diff %u", ourTicks - newClientTicks);
    }
    ...
    for (std::list<uint16>::iterator itr = _currentChecks.begin(); itr != _currentChecks.end(); ++itr)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    TC_LOG_INFO("warden", "RESULT MEM_CHECK not 0x00, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                    continue;
                }
                ...
                {
                    TC_LOG_INFO("warden", "RESULT MEM_CHECK fail CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                ...
                buff.rpos(buff.rpos() + rd->Length);
                TC_LOG_INFO("warden", "RESULT MEM_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) != 0)
                {
                    if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                        TC_LOG_INFO("warden", "RESULT PAGE_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    if (type == MODULE_CHECK)
                        TC_LOG_INFO("warden", "RESULT MODULE_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    if (type == DRIVER_CHECK)
                        TC_LOG_INFO("warden", "RESULT DRIVER_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                ...
                buff.rpos(buff.rpos() + 1);
                if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                    TC_LOG_INFO("warden", "RESULT PAGE_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                else if (type == MODULE_CHECK)
                    TC_LOG_INFO("warden", "RESULT MODULE_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                else if (type == DRIVER_CHECK)
                    TC_LOG_INFO("warden", "RESULT DRIVER_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
            case LUA_STR_CHECK:
            {
                ...
                if (Lua_Result != 0)
                {
                    TC_LOG_INFO("warden", "RESULT LUA_STR_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                    continue;
                }
                ...
                if (luaStrLen != 0)
                {
                    ...
                    TC_LOG_INFO("warden", "Lua string: %s", str);
                    delete[] str;
                }
                buff.rpos(buff.rpos() + luaStrLen);         // Skip string
                TC_LOG_INFO("warden", "RESULT LUA_STR_CHECK passed, CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    TC_LOG_INFO("warden", "RESULT MPQ_CHECK not 0x00 account id %u", _session->GetAccountId());
                    checkFailed = *itr;
                    continue;
                }
                ...
                {
                    TC_LOG_INFO("warden", "RESULT MPQ_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                ...
                buff.rpos(buff.rpos() + 20);                // 20 bytes SHA1
                TC_LOG_INFO("warden", "RESULT MPQ_CHECK passed, CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
