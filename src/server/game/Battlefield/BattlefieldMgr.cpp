...
{
    m_UpdateTimer = 0;
    //TC_LOG_INFO("bg.battlefield", "Instantiating BattlefieldMgr");
}

BattlefieldMgr::~BattlefieldMgr()
{
    //TC_LOG_INFO("bg.battlefield", "Deleting BattlefieldMgr");
...
void BattlefieldMgr::InitBattlefield()
{
    ...
    /* For Cataclysm: Tol Barad
       ...
       if (!pBf->SetupBattlefield())
       {
       TC_LOG_INFO("bg.battlefield", "Battlefield : Tol Barad init failed.");
       delete pBf;
       }
       else
       {
       m_BattlefieldSet.push_back(pBf);
       TC_LOG_INFO("bg.battlefield", "Battlefield : Tol Barad successfully initiated.");
       } */
}
...
void BattlefieldMgr::HandlePlayerEnterZone(Player* player, uint32 zoneid)
{
    ...
    TC_LOG_INFO("bg.battlefield", "Player %u entered outdoorpvp id %u", player->GetGUIDLow(), bf->GetTypeId());
}
...
void BattlefieldMgr::HandlePlayerLeaveZone(Player* player, uint32 zoneid)
{
    ...
    TC_LOG_INFO("bg.battlefield", "Player %u left outdoorpvp id %u", player->GetGUIDLow(), itr->second->GetTypeId());
}
