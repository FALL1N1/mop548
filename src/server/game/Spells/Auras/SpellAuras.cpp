...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false), _effMask(0)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        if (slot < MAX_AURAS)
        {
            ...
            SetNeedClientUpdate();
            TC_LOG_INFO("spells", "Aura: %u Effect: %d put to unit visible auras slot: %u", GetBase()->GetId(), GetEffectMask(), slot);
        }
        else
            TC_LOG_INFO("spells", "Aura: %u Effect: %d could not find empty unit visible slot", GetBase()->GetId(), GetEffectMask());
    }
...
void AuraApplication::_HandleEffect(uint8 effIndex, bool apply)
{
    ...
    TC_LOG_INFO("spells", "AuraApplication::_HandleEffect: %u, apply: %u: amount: %u", aurEff->GetAuraType(), apply, aurEff->GetAmount());
...
{
    if (m_spellInfo->GetSpellPowerCost(caster).ManaPerSecond)
...
void Aura::Update(uint32 diff, Unit* caster)
{
    if (m_duration > 0)
    {
        ...
        if (m_timeCla)
        {
            ...
            {
                if (int32 manaPerSecond = m_spellInfo->GetSpellPowerCost(caster).ManaPerSecond)
                {
                    m_timeCla += 1000 - diff;

                    Powers powertype = Powers(m_spellInfo->GetSpellPowerCost(caster).PowerType);
...
void Aura::RefreshDuration()
{
    ...
    if (m_spellInfo->GetSpellPowerCost(GetCaster()).ManaPerSecond)
        m_timeCla = 1 * IN_MILLISECONDS;
}
...
bool Aura::CanBeSentToClient() const
{
    return !IsPassive() || GetSpellInfo()->HasAreaAuraEffect() || HasEffectType(SPELL_AURA_ABILITY_IGNORE_AURASTATE) || HasEffectType(SPELL_AURA_CAST_WHILE_WALKING)
        || HasEffectType(SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS) || HasEffectType(SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS_2) || HasEffectType(SPELL_AURA_MOD_IGNORE_SHAPESHIFT) || HasEffectType(SPELL_AURA_MOD_CHARGES);
}
...
void Aura::LoadScripts()
{
    ...
    for (std::list<AuraScript*>::iterator itr = m_loadedScripts.begin(); itr != m_loadedScripts.end();)
    {
        ...
        TC_LOG_INFO("spells", "Aura::LoadScripts: Script `%s` for aura `%u` is loaded now", (*itr)->_GetScriptName()->c_str(), m_spellInfo->Id);
        (*itr)->Register();
        ++itr;
    }
}
