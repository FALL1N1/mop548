...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNULL,                                      //343 SPELL_AURA_MOD_AUTOATTACK_DAMAGE_TARGET
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //411 SPELL_AURA_MOD_CHARGES
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        target->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PREVENT_EMOTES);            // blizz like 2.0.x
    ...
    else
    {
        ...
        */
        ...
        target->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PREVENT_EMOTES);         // blizz like 2.0.x
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            switch (GetId())
            {
                ...
                case 93341: // Guardians of Hyjal
                case 93347: // Therazane
                case 93339: // The Earthen Ring
                case 93337: // Ramkahen
                case 93368: // Wildhammer Clan (Aliance only)
                case 94158: // Dragonmaw Clan (Horde only)
                case 93816: // Gilneas
                case 93830: // Bilgewater Cartel
                case 93825: // Orgrimmar
                case 93795: // Stormwind
                case 94462: // Undercity
                case 93806: // Darnasus
                case 93827: // Darkspear
                case 93805: // Ironforge
                case 93828: // Silvermoon
                case 93811: // Exodar
                case 94463: // Thunder Bluff
                case 93821: // Gnomeregan
                {
                    if (!caster || caster->GetTypeId() != TYPEID_PLAYER)
                        break;
                    ...
                    if (apply)
                    {
                        switch (m_spellInfo->Id)
                        {
                            ...
                            case 93368: FactionID = 1174; break; // Wildhammer Clan (Aliance only)
                            case 94158: FactionID = 1172; break; // Dragonmaw Clan (Horde only)
                            ...
                            case 93806: FactionID = 69;   break; // Darnasus
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (triggeredSpellInfo)
    {
        if (Unit* triggerCaster = triggeredSpellInfo->NeedsToBeTriggeredByCaster(m_spellInfo) ? caster : target)
        {
            ...
            TC_LOG_INFO("spells", "AuraEffect::HandlePeriodicTriggerSpellAuraTick: Spell %u Trigger %u", GetId(), triggeredSpellInfo->Id);
        }
    }
    else
    {
        ...
        if (!c || !caster || !sScriptMgr->OnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()), target->ToCreature()) ||
            ...
            TC_LOG_INFO("spells", "AuraEffect::HandlePeriodicTriggerSpellAuraTick: Spell %u has non-existent spell %u in EffectTriggered[%d] and is therefor not triggered.", GetId(), triggerSpellId, GetEffIndex());
    }
}
...
void AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        if (Unit* triggerCaster = triggeredSpellInfo->NeedsToBeTriggeredByCaster(m_spellInfo) ? caster : target)
        {
            ...
            TC_LOG_INFO("spells", "AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick: Spell %u Trigger %u", GetId(), triggeredSpellInfo->Id);
        }
    }
    else
        TC_LOG_INFO("spells","AuraEffect::HandlePeriodicTriggerSpellWithValueAuraTick: Spell %u has non-existent spell %u in EffectTriggered[%d] and is therefor not triggered.", GetId(), triggerSpellId, GetEffIndex());
}
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        damage = caster->SpellDamageBonusDone(target, GetSpellInfo(), damage, DOT, m_effIndex, GetBase()->GetStackAmount());
        damage = target->SpellDamageBonusTaken(caster, GetSpellInfo(), damage, DOT, m_effIndex, GetBase()->GetStackAmount());
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (caster->IsAlive())
    {
        ...
        uint32 heal = uint32(caster->SpellHealingBonusDone(caster, GetSpellInfo(), uint32(new_damage * gainMultiplier), DOT, m_effIndex, GetBase()->GetStackAmount()));
        heal = uint32(caster->SpellHealingBonusTaken(caster, GetSpellInfo(), heal, DOT, m_effIndex, GetBase()->GetStackAmount()));
...
void AuraEffect::HandlePeriodicHealthFunnelAuraTick(Unit* target, Unit* caster) const
{
    ...
    TC_LOG_INFO("spells", "PeriodicTick: donator %u target %u damage %u.", caster->GetEntry(), target->GetEntry(), damage);
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    else
    {
        ...
        damage = caster->SpellHealingBonusDone(target, GetSpellInfo(), damage, DOT, m_effIndex, GetBase()->GetStackAmount());
        damage = target->SpellHealingBonusTaken(caster, GetSpellInfo(), damage, DOT, m_effIndex, GetBase()->GetStackAmount());
    }
    ...
    {
        uint32 funnelDamage = GetSpellInfo()->GetSpellPowerCost(caster).ManaPerSecond; // damage is not affected by spell power
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (m_spellInfo->GetSpellPowerCost(caster).ManaCostPercentage)
    {
        // max value
...
void AuraEffect::HandleProcTriggerSpellAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        TC_LOG_INFO("spells", "AuraEffect::HandleProcTriggerSpellAuraProc: Triggering spell %u from aura %u proc", triggeredSpellInfo->Id, GetId());
        triggerCaster->CastSpell(triggerTarget, triggeredSpellInfo, true, NULL, this);
    }
    else
        TC_LOG_INFO("spells","AuraEffect::HandleProcTriggerSpellAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in Spell.dbc.", triggerSpellId, GetId());
}
...
void AuraEffect::HandleProcTriggerSpellWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        ...
        TC_LOG_INFO("spells", "AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Triggering spell %u with value %d from aura %u proc", triggeredSpellInfo->Id, basepoints0, GetId());
        triggerCaster->CastCustomSpell(triggerTarget, triggerSpellId, &basepoints0, NULL, NULL, true, NULL, this);
    }
    else
        TC_LOG_INFO("spells","AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in Spell.dbc.", triggerSpellId, GetId());
}
...
void AuraEffect::HandleProcTriggerDamageAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    TC_LOG_INFO("spells", "AuraEffect::HandleProcTriggerDamageAuraProc: Triggering %u spell damage from aura %u proc", damage, GetId());
...
void AuraEffect::HandleRaidProcFromChargeAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    switch (GetId())
    {
        ...
        case 43593:            // Cold Stare
            ...
            break;
        default:
            TC_LOG_INFO("spells", "AuraEffect::HandleRaidProcFromChargeAuraProc: received not handled spell: %u", GetId());
            return;
    }
    ...
    TC_LOG_INFO("spells", "AuraEffect::HandleRaidProcFromChargeAuraProc: Triggering spell %u from aura %u proc", triggerSpellId, GetId());
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    {
        TC_LOG_INFO("spells", "AuraEffect::HandleRaidProcFromChargeWithValueAuraProc: received not handled spell: %u", GetId());
        return;
    }
    ...
    TC_LOG_INFO("spells", "AuraEffect::HandleRaidProcFromChargeWithValueAuraProc: Triggering spell %u from aura %u proc", triggerSpellId, GetId());
