...
{
    TC_LOG_INFO("server.authserver", "'%s:%d' Accepting connection", socket().getRemoteAddress().c_str(), socket().getRemotePort());
}

void AuthSocket::OnClose(void)
{
    TC_LOG_INFO("server.authserver", "AuthSocket::OnClose");
}
...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < AUTH_TOTAL_COMMANDS; ++i)
        {
            ...
            {
                TC_LOG_INFO("server.authserver", "Got data for cmd %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());

                if (!(*this.*table[i].handler)())
                {
                    TC_LOG_INFO("server.authserver", "Command handler failed for cmd %u recv length %u", (uint32)_cmd, (uint32)socket().recv_len());
                    return;
                }
                break;
            }
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleLogonChallenge");
    ...
    TC_LOG_INFO("server.authserver", "[AuthChallenge] got header, body is %#04x bytes", remaining);
    ...
    TC_LOG_INFO("server.authserver", "[AuthChallenge] got full packet, %#04x bytes", ch->size);
    TC_LOG_INFO("server.authserver", "[AuthChallenge] name(%d): '%s'", ch->I_len, ch->I);
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] Banned ip tries to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort());
    }
    else
    {
        ...
        if (res2)
        {
            ...
            {
                TC_LOG_INFO("server.authserver", "[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[3].GetCString());
                TC_LOG_INFO("server.authserver", "[AuthChallenge] Player address is '%s'", ip_address.c_str());

                if (strcmp(fields[4].GetCString(), ip_address.c_str()) != 0)
                {
                    TC_LOG_INFO("server.authserver", "[AuthChallenge] Account IP differs");
                    pkt << uint8(WOW_FAIL_LOCKED_ENFORCED);
                    locked = true;
                }
                else
                    TC_LOG_INFO("server.authserver", "[AuthChallenge] Account IP matches");
            }
            else
            {
                TC_LOG_INFO("server.authserver", "[AuthChallenge] Account '%s' is not locked to ip", _login.c_str());
                std::string accountCountry = fields[3].GetString();
                if (accountCountry.empty() || accountCountry == "00")
                    TC_LOG_INFO("server.authserver", "[AuthChallenge] Account '%s' is not locked to country", _login.c_str());
                else if (!accountCountry.empty())
                {
                    ...
                    if (PreparedQueryResult sessionCountryQuery = LoginDatabase.Query(stmt))
                    {
                        ...
                        TC_LOG_INFO("server.authserver", "[AuthChallenge] Account '%s' is locked to country: '%s' Player country is '%s'", _login.c_str(), accountCountry.c_str(), loginCountry.c_str());
                        if (loginCountry != accountCountry)
                        {
                            TC_LOG_INFO("server.authserver", "[AuthChallenge] Account country differs.");
                            pkt << uint8(WOW_FAIL_UNLOCKABLE_LOCK);
                            locked = true;
                        }
                        else
                            TC_LOG_INFO("server.authserver", "[AuthChallenge] Account country matches");
                    }
                    else
                        TC_LOG_INFO("server.authserver", "[AuthChallenge] IP2NATION Table empty");
                }
            }
            ...
            if (!locked)
            {
                ...
                if (banresult)
                {
                    ...
                    {
                        pkt << uint8(WOW_FAIL_BANNED);
                        TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] Banned account %s tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
                    }
                    else
                    {
                        pkt << uint8(WOW_FAIL_SUSPENDED);
                        TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] Temporarily banned account %s tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
                    }
                }
                else
                {
                    ...
                    TC_LOG_INFO("network", "database authentication values: v='%s' s='%s'", databaseV.c_str(), databaseS.c_str());
                    ...
                    TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' locale (%u)", socket().getRemoteAddress().c_str(), socket().getRemotePort(),
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    {
        TC_LOG_INFO("server.authserver", "'%s:%d' User '%s' successfully authenticated", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
    ...
    else
    {
        ...
        TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] account %s tried to login with invalid password!", socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str ());
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] account %s got banned for '%u' seconds because it failed to authenticate '%u' times",
                    ...
                    else
                    {
                        ...
                        TC_LOG_INFO("server.authserver", "'%s:%d' [AuthChallenge] IP %s got banned for '%u' seconds because account %s failed to authenticate '%u' times",
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleReconnectChallenge");
    if (socket().recv_len() < sizeof(sAuthLogonChallenge_C))
        return false;
    ...
    uint16 remaining = ((sAuthLogonChallenge_C *)&buf[0])->size;
    TC_LOG_INFO("server.authserver", "[ReconnectChallenge] got header, body is %#04x bytes", remaining);
    ...
    // Read the remaining of the packet
    socket().recv((char *)&buf[4], remaining);
    TC_LOG_INFO("server.authserver", "[ReconnectChallenge] got full packet, %#04x bytes", ch->size);
    TC_LOG_INFO("server.authserver", "[ReconnectChallenge] name(%d): '%s'", ch->I_len, ch->I);
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleReconnectProof");
    // Read the packet
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleRealmList");
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleXferResume");
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleXferCancel");
...
{
    TC_LOG_INFO("server.authserver", "Entering _HandleXferAccept");
...
void Patcher::LoadPatchMD5(char *szFileName)
{
    ...
    TC_LOG_INFO("network", "Loading patch info from %s\n", path.c_str());
