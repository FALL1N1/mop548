...
namespace VMAP
{
    ...
    class AreaInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                TC_LOG_INFO("maps", "AreaInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    class LocationInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                TC_LOG_INFO("maps", "LocationInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    */
    ...
    bool StaticMapTree::InitMap(const std::string &fname, VMapManager2* vm)
    {
        ...
        ModelSpawn spawn;
#ifdef VMAP_DEBUG
        TC_LOG_INFO("maps", "StaticMapTree::InitMap() : map isTiled: %u", static_cast<uint32>(iIsTiled));
#endif
                ...
                if (result)
                {
                    ...
                    if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
                    {
                        if (!iLoadedSpawns.count(referencedVal))
                        {
                            ...
                            {
                                TC_LOG_INFO("maps", "StaticMapTree::LoadMapTile() : invalid tree element (%u/%u)", referencedVal, iNTreeValues);
                                continue;
                            }
#endif
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                TC_LOG_INFO("maps", "StaticMapTree::LoadMapTile() : trying to load wrong spawn in node");
                            else if (iTreeValues[referencedVal].name != spawn.name)
                                TC_LOG_INFO("maps", "StaticMapTree::LoadMapTile() : name collision on GUID=%u", spawn.ID);
#endif
                        }
                    }
                    else
