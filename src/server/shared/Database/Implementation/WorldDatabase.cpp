...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_MASTERIES, "SELECT ID, base, ratio FROM masteries", CONNECTION_SYNCH);

    // custom
    PrepareStatement(WORLD_INS_UNK_OPCODES, "INSERT IGNORE INTO unk_opcodes (id, type, name) VALUES (CONCAT('0x',HEX(?)), ?, ?)", CONNECTION_ASYNC);
}
