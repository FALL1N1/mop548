                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        for (auto itr : tempList)
                        {
                            ...
                            if (itr->IsImmunedToSpell(sSpellMgr->GetSpellInfo(DK_SPELL_DEATH_GRIP_ONLY_JUMP)))
                                continue;
							/*
                            if (_player->IsVisionObscured(itr))
                                continue;
							*/
    ...
    public:
        ...
        class spell_dk_runic_corruption_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (AuraEffect* runicCorruption = _player->GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 4068, 0))
                    {
                        if (roll_chance_i(45))
                        {
...
class spell_dk_necrotic_strike : public SpellScriptLoader
{
    public:
        ...
        class spell_dk_necrotic_strike_AuraScript : public AuraScript
        {
			...
			void CalculateAmount(AuraEffect* aurEff, int32 & amount, bool & canBeRecalculated, SpellEffIndex, AuraType)
            {
                if (GetCaster())
                {
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        for (uint32 i = 0; i < MAX_RUNES; ++i)
                        {
                            ...
                            if (!_player->GetRuneCooldown(i) && rune == RUNE_DEATH)
                            {
                                ...
                                TC_LOG_WARN("server.worldserver", "Necrotic: %u - %u", i, cooldown);
            ...
            SpellCastResult CheckRunes()
            {
                if (GetCaster() && GetCaster()->ToPlayer())
                    for (uint8 i = 0; i < MAX_RUNES; ++i)
                        ...
                        {
                            if (GetCaster()->ToPlayer()->GetRuneConvertSpell(i) == 123683)
                                continue;
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        while (!_selected)
                        {
                            ...
                            if (_player->GetRuneCooldown(runeRandom))
                            {
                                ...
                                _player->SetDeathRuneUsed(runeRandom, false);
                                _player->AddRuneBySpell(runeRandom, RUNE_DEATH, 123693);
                                //_player->ConvertRune(runeRandom, RUNE_DEATH);
                                _player->ResyncRunes(MAX_RUNES);
                                TC_LOG_WARN("server.worldserver", "Plague - %u _ %u", runeRandom, _player-> GetRuneCooldown(runeRandom));
                                _selected = true;
                            }
                        }
                    }
        ...
        class spell_dk_scourge_strike_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* unitTarget = GetHitUnit())
                {
                    ...
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_DK_ITEM_T8_MELEE_4P_BONUS, EFFECT_0))
    ...
    public:
        ...
        class spell_dk_death_grip_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    if (!target->HasAuraType(SPELL_AURA_DEFLECT_SPELLS) && !target->HasAuraType(SPELL_AURA_MOD_ROOT)) // Deterrence
                    {
