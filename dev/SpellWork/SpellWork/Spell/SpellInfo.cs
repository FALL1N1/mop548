    ...
    class SpellInfo
    {
        ...
        private void ViewSpellInfo()
        {
            ...
            if (_spell.AttributesEx13 != 0)
                _rtb.AppendFormatLine("AttributesEx13: 0x{0:X8} ({1})", _spell.AttributesEx13, _spell.AttributesEx13);
        ...
        private void AppendSpellEffectInfo()
        {
            ...
            foreach (var effect in _spell.Effects)
            {
                ...
                if (effect.ScalingMultiplier != 0.0f && _spell.Scaling != null && _spell.Scaling.PlayerClass != 0)
                {
                    ...
                    uint ClassIndex = 0;
                    switch (_spell.Scaling.PlayerClass)
                    { 
                        case -1:
                            ClassIndex = 11;
                            break;
                        ...
                        case 1000:
                            ...
                            break;
                        default:
                            ClassIndex = (uint)_spell.Scaling.PlayerClass - 1;
                            break;
                    }
                    var gtEntry = (uint)(ClassIndex * 100) + DBC.DBC.SelectedLevel - 1;
